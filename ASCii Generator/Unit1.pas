...
uses
  ...
  Vcl.ComCtrls, Vcl.Shell.ShellCtrls, Vcl.Samples.Spin, Vcl.Menus,
  Bumpmapping, PngImage, GIFImg, ShellApi, Jpeg, Vcl.ExtCtrls, Vcl.StdCtrls;
...
type
  TForm1 = class(TForm)
    ...
    procedure Bumpmap2Click(Sender: TObject);
    ...
    procedure SpecialChars1Click(Sender: TObject);
    procedure Alphabet1Click(Sender: TObject);
...
begin
  ...
  Save1.Enabled := false;
  ReloadPicture1.Enabled := false;
  Clear1.Enabled := false;
  Invert1.Enabled := false;
  Resample1.Enabled := false;
  Highlight1.Enabled := false;
  Blur1.Enabled := false;
  Sepia1.Enabled := false;
  BumpMap1.Enabled := false;
  Color1.Enabled := false;
  Trz1.Enabled := false;
  FlipX1.Enabled := false;
  FlipY1.Enabled := false;
...
begin
  ...
  Save1.Enabled := true;
  ReloadPicture1.Enabled := true;
  Clear1.Enabled := true;
  Invert1.Enabled := true;
  Resample1.Enabled := true;
  Highlight1.Enabled := true;
  Blur1.Enabled := true;
  Sepia1.Enabled := true;
  BumpMap1.Enabled := true;
  Color1.Enabled := true;
  Trz1.Enabled := true;
  FlipX1.Enabled := true;
  FlipY1.Enabled := true;
...
procedure TForm1.Alphabet1Click(Sender: TObject);
begin
...
begin
  if BumpMap1.Checked = false then
  begin
    ReloadPicture1.Click;
    Exit;
  end;
...
procedure TForm1.Bumpmap2Click(Sender: TObject);
var
  R, G, B: Byte;
  i : integer;
begin
...
begin
  Label5.Left := ScrollBox1.Width div 2-15;
  Label5.Top := ScrollBox1.Height div 2;
end;
...
begin
  ...
  Label5.Left := ScrollBox1.Width div 2-10;
  Label5.Top := ScrollBox1.Height div 2;
end;
...
begin
  ...
  case RadioGroup1.ItemIndex of
    0 : Red1.Checked := true;
    1 : Green1.Checked := true;
    2 : Blue1.Checked := true;
    3 : Aqua1.Checked := true;
    4 : Pink1.Checked := true;
    5 : Yellow1.Checked := true;
    6 : Gray1.Checked := true;
    7 : RGB1.Checked := true;
  end;
end;
...
procedure TForm1.SpecialChars1Click(Sender: TObject);
begin
...
begin
  if Stretch1.Checked = false then begin
...
begin
    ...
    begin  for x := 0 to MaxCharW do
      ...
      aColor := Canvas.Pixels[nx, ny];
         ...
         Case RadioGroup1.ItemIndex of
          ...
          3: bmp.Canvas.Font.Color := RGB(0, CalcGrayValue(aColor), CalcGrayValue(aColor)); // Aqua
          4: bmp.Canvas.Font.Color := RGB(CalcGrayValue(aColor), 0, CalcGrayValue(aColor)); // Pink
          5: bmp.Canvas.Font.Color := RGB(CalcGrayValue(aColor), CalcGrayValue(aColor), 0); // Yellow
          6: bmp.Canvas.Font.Color := RGB(CalcGrayValue(aColor), CalcGrayValue(aColor), CalcGrayValue(aColor));
          7: bmp.Canvas.Font.Color := aColor; // Calculate RGB Complete to Font
        end;
        ...
        if (RadioGroup1.Buttons[0].Checked = false) and
           ...
           (RadioGroup1.Buttons[4].Checked = false) and
           (RadioGroup1.Buttons[5].Checked = false) and
           (RadioGroup1.Buttons[6].Checked = false) and
           (RadioGroup1.Buttons[7].Checked = false) then begin
          ...
          // Special characters
          if RadioButton4.Checked = true then begin  // Special characters
            bmp.Canvas.TextOut(nx, ny, CHR( Ord('!') + Random(10) ));
          end;

          // Alphabet
          if RadioButton5.Checked = true then begin  // Alphabet characters
            bmp.Canvas.TextOut(nx, ny, CHR( Ord('a') + Random(26) ));
          end;

          // Alphabet
          if RadioButton6.Checked = true then begin  // Alphabet characters
            bmp.Canvas.TextOut(nx, ny, CHR( Ord('0') + Random(9) ));
          end;
  ...
  if Draw1.Checked = true then
  begin
    Image1.Canvas.Pen.Color := ColorDialog2.Color;

    if x11.Checked = true then Image1.Canvas.Pen.Width := 1;
    if x21.Checked = true then Image1.Canvas.Pen.Width := 2;
    if x31.Checked = true then Image1.Canvas.Pen.Width := 3;
    if x41.Checked = true then Image1.Canvas.Pen.Width := 4;
    if x51.Checked = true then Image1.Canvas.Pen.Width := 5;
    if x61.Checked = true then Image1.Canvas.Pen.Width := 6;
    if x71.Checked = true then Image1.Canvas.Pen.Width := 7;
    if x81.Checked = true then Image1.Canvas.Pen.Width := 8;
    if x91.Checked = true then Image1.Canvas.Pen.Width := 9;
    if x101.Checked = true then Image1.Canvas.Pen.Width := 10;
    ...
    Image1.Canvas.MoveTo(Image1.Width-1, Image1.Height-1);
    Image1.Canvas.LineTo(Image1.Width-1, 1);
    Image1.Canvas.MoveTo(Image1.Width-1, 1);
...
var
  str : string;
begin
  InputQuery('ASCii Art Char',
             'Type chars which should be generated for ASCII:', str);
